...
volumes.forEach(vol => {
  ...
  const replacement = `<button onClick={() => handleVolume('${vol[0]}')} className="w-full text-left p-5 sm:p-6 lg:p-7 rounded-2xl border border-slate-200 hover:border-brand-500 hover:bg-brand-50 hover:shadow-md transition-all group flex flex-col justify-between h-full bg-white relative">
                         <div className="mb-4">
                           <div className="font-bold text-base md:text-lg lg:text-xl text-slate-800 group-hover:text-brand-700 leading-tight mb-2 sm:mb-3 flex flex-row justify-between items-start w-full gap-2 border-b border-transparent group-hover:border-brand-100 pb-2 transition-colors">
                             <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between w-full gap-1 sm:gap-4">
                               ...
                               ${vol[2] ? `<span className="text-xs sm:text-sm font-medium text-slate-500 whitespace-nowrap bg-slate-50 px-2 py-0.5 rounded-md group-hover:bg-brand-100/50 group-hover:text-brand-600 transition-colors">${vol[2]}</span>` : ''}
                             </div>
                           </div>
                           <div className="text-xs sm:text-sm md:text-base text-slate-500 leading-relaxed text-balance w-full text-left">${vol[3]}</div>
                         </div>
...
// Note: handle text-xs for descriptions
content = content.replace(/<div className="text-xs text-slate-500">/g, '<div className="text-sm md:text-base text-slate-500 mt-1">');
// Note: handle button class
content = content.replace(/className="w-full text-left p-3 sm:p-4 rounded-xl border border-slate-200 hover:border-brand-500 hover:bg-brand-50 transition-colors group flex items-center justify-between"/g, 'className="w-full text-left p-4 md:p-6 lg:p-7 rounded-2xl border border-slate-200 hover:border-brand-500 hover:bg-brand-50 hover:shadow-md transition-all group flex items-center justify-between bg-white relative"');
