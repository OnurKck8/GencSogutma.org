...
const volumes = [
  ...
  ['v4', 'Panelvan <span className="sm:hidden text-xs block mt-0.5">Ekstra</span>', '(17+ m³)', 'En büyük hacimli panelvan araçlar.'],
...
volumes.forEach(vol => {
  ...
  const replacement = `<button onClick={() => handleVolume('${vol[0]}')} className="w-full text-center sm:text-left p-1.5 sm:p-4 rounded-xl border border-slate-200 hover:border-brand-500 hover:bg-brand-50 transition-colors group flex flex-col items-center sm:items-start justify-center sm:justify-between h-full gap-1 sm:gap-0 aspect-square sm:aspect-auto bg-white sm:bg-transparent shadow-sm sm:shadow-none">
                         <div className="mb-0 sm:mb-2 flex flex-col items-center sm:block w-full">
                           <div className="font-bold text-[11px] xs:text-[12px] sm:text-base text-slate-800 group-hover:text-brand-700 leading-tight sm:mb-1 flex flex-col sm:flex-row justify-center sm:justify-between items-center w-full gap-0.5 sm:gap-0 mt-1 sm:mt-0">
                             <span className="text-center text-balance">${vol[1]}</span> 
                             <span className="text-[9px] sm:text-sm font-semibold sm:font-medium text-slate-500 mt-1 sm:mt-0 px-1 py-0.5 sm:px-0 sm:py-0 border sm:border-0 rounded-md sm:rounded-none border-slate-100 bg-slate-50 sm:bg-transparent tracking-tighter sm:tracking-normal w-max">${vol[2]}</span>
                           </div>
